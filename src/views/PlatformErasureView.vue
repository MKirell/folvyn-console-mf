<template>
  <div class="mx-auto w-full max-w-[1180px]">
    <PlatformHeader
      title="Erasure queue"
      description="Every request, its thirty-day clock, and what the cascade actually removed."
    />
      ...
      >
        <ul v-if="rows.length" class="flex min-h-0 flex-1 flex-col gap-2" role="list">
          <li
            v-for="row in rows"
            :key="row.id"
            class="flex flex-wrap items-center gap-3 rounded-[9px] border px-3 py-2.5"
            :class="tone(row)"
          >
            ...
            <AppButton
              ...
              @click="run(row)"
              >{{ row.state === 'failed' ? 'Retry' : 'Run now' }}</AppButton
            >

            <p class="basis-full text-[0.74rem] text-muted">
        ...
        <div v-else class="grid flex-1 place-items-center py-6 text-center">
          <div>
            <p class="text-[0.86rem] text-ink">No erasure is waiting.</p>
